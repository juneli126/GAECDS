...
for epoch_gae in range(5):
    ...
    for negative_i in range(4):
        if negative_i != 3:
            x_new_matrix_i, new_label = negative_analysic(x_new_matrix,drug_label,1344*(negative_i+1))
            new_label = np.array(new_label)
        else:
            x_new_matrix_i = x_new_matrix
            new_label = drug_label


        x_new_matrix_i = np.array(x_new_matrix_i)
        print(x_new_matrix_i.shape)

        x_new_matrix_i = torch.tensor(x_new_matrix_i)
        x_new_matrix_i = torch.reshape(x_new_matrix_i, (-1, 128, 1, 1))


        # five fold cross validation
        lenth = len(x_new_matrix_i)
        pot = int(lenth / 5)
        print('lenth', lenth)
        print('pot', pot)
        ...
        for i_time in range(5):
            ...
            x_train = x_new_matrix_i[train_num]
            x_test = x_new_matrix_i[test_num]

            y_train = new_label[train_num]
            y_test = new_label[test_num]
            ...
            for epoch_cnn in range(200):
                ...
                print('Epoch GAE : ',epoch_gae,'| data time: ', negative_i, '| cross validation: ',i_time,'| Epoch: ', epoch_cnn, '|loss: ', loss_net.item(),'| accuracy_train: ',acc_train, '| accuracy_test: ', acc_test)
            metrics_scores = metrics_draw(y_test, pred_test, '0.0001cell_epoch%s_' % epoch_gae)
            for i in range(len(pred_test)):
                with open('pred_test_GAEC_all_cell%s_epoch%s_%s.txt' % (negative_i,epoch_gae,i_time), 'a') as f:
...
torch.save(net,'net_cnn_all_negative.pt')
print('Save model!')
