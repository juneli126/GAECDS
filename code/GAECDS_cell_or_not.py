...
for epoch_gae in range(5):
    ...
    for cell_i in range(2):
        if cell_i == 0:
            ...
            x_new_matrix = new_matrix(drug1,drug2,x_matrix)
        ...
        for i_time in range(5):
            ...
            for epoch_cnn in range(200):
                ...
                print('cell or not: ',cell_i,'epoch_GAE: ',epoch_gae, 'cross validation: ', i_time, 'Epoch: ', epoch_cnn, '|loss: ', loss_net.item(), '| accuracy_train: ',
                      acc_train, '| accuracy_test: ', acc_test)


            metrics_scores = metrics_draw(y_test, pred_test, '0.0001cell_epoch%s_' % epoch_gae)
            for i in range(len(pred_test)):
                with open('pred_test_GAEC_cell_time%i_epoch%s_%s.txt' % (cell_i,epoch_gae,i_time), 'a') as f:
...
torch.save(net,'net_cnn_cell_or_not.pt')
print('Save model!')
