...
for epoch_gae in range(5):
    ...
    for i_time in range(5):
        ...
        for epoch_cnn in range(200):
            ...
            print('epoch_GAE: ',epoch_gae, 'cross validation: ', i_time, 'Epoch: ', epoch_cnn, '|loss: ', loss_net.item(), '| accuracy_train: ',
                  acc_train, '| accuracy_test: ', acc_test)


        metrics_scores = metrics_draw(y_test, pred_test, '0.0001cell_epoch%s_' % epoch_gae)
        for i in range(len(pred_test)):
            with open('pred_test_GAEC_cell_all_epoch%s_%s.txt' % (epoch_gae,i_time), 'a') as f:
...
torch.save(net,'net_cnn_GAECDS.pt')
print('Save model!')
